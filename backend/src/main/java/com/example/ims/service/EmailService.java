...
public class EmailService {
    ...
    public void sendClaimNotificationEmail(String toEmail, com.example.ims.entity.Claim claim) {
        ...
        String subject = "클레임 접수 안내 (" + prodName + ") - " + replyDeadline + "까지 회신";
    ...
    public void sendProductionAuditNotificationEmail(String toEmail, com.example.ims.entity.Product product) {
        String subject = "신제품 생산감리 사진 등록 요청 (" + product.getProductName() + ")";
    ...
    public boolean sendCustomEmail(String toEmail, String subject, String body) {
        ...
        if (!subject.startsWith("[QMS 시스템]")) {
            subject = "[QMS SYSTEM] " + subject;
        }

        if (!emailSender.isConfigured()) {
            log.info("==== [MOCK CUSTOM EMAIL SEND (EmailSender not configured)] ====");
            log.info("To: {}", toEmail);
        ...
        mailExecutor.execute(() -> {
            try {
                emailSender.send(toEmail, finalSubject, finalHtmlBody);
            } catch (Exception e) {
                handleMailFailure(toEmail, finalSubject, e);
            }
        });
        return false;
    }
