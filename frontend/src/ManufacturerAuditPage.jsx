...
const ManufacturerAuditPage = ({ user }) => {
    ...
    const handleExportPdf = async (id) => {
        try {
            const res = await exportAuditToPdf(id);
        ...
        { 
            ...
            cellRenderer: (p) => (
                <div style={{ display: 'flex', gap: '5px', alignItems: 'center', height: '100%' }}>
                    ...
                    <button onClick={() => handleExportPdf(p.data.id)} className="outline" style={{ padding: '2px 8px', fontSize: '11px' }}>PDF</button>
            ...
            {isModalOpen && (
                <div className="drawer-overlay" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.6)' }} onClick={(e) => e.target === e.currentTarget && setIsModalOpen(false)}>
                    <div className="card" style={{ width: '1200px', maxWidth: '98vw', maxHeight: '92vh', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                        <div style={{ padding: '20px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc' }}>
                            <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 'bold' }}>{selectedAudit ? 'Audit 내역 수정' : '신규 Audit 등록'}</h3>
                            <button onClick={() => setIsModalOpen(false)} style={{ fontSize: '24px', background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}>&times;</button>
                        </div>

                        <div style={{ flex: 1, overflowY: 'auto', padding: '25px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginBottom: '25px', padding: '15px', background: '#f1f5f9', borderRadius: '8px' }}>
                                <div>
